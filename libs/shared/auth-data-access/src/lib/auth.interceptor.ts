...
export const authInterceptor: HttpInterceptorFn = (request, next) => {
  ...
  const targetsApi = requestUrl.origin === apiUrl.origin && requestUrl.pathname.startsWith(apiUrl.pathname);

  if (!token || !targetsApi || request.context.get(SKIP_AUTH)) return next(request);
